...
class EsperaQueueError(RuntimeError):
    """Indicates processing errors."""
    pass


class EsperaQueue(threading.Thread):
    """
    ...
    Args:
        ...
        name (:obj:`str`, optional): Thread's name. Defaults to ``'EsperaQueue-N'``, where N is
    ...
    def __init__(self,
        ...
        super(EsperaQueue, self).__init__(name=name)
        self.daemon = False
        if autostart:  # immediately start processing
            super(EsperaQueue, self).start()
    ...
    def stop(self, timeout=None):
        ...
        super(EsperaQueue, self).join(timeout=timeout)
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        if not self.is_alive() or self.__exit_req:
            raise EsperaQueueError('Could not process callback in stopped thread')
...
class MessageQueue(object):
    ...
    Contains two ``EsperaQueue``, for group and for all messages, interconnected in delay chain.
    Callables are processed through *group* ``EsperaQueue``, then through *all* ``EsperaQueue`` for
    group-type messages. For non-group messages, only the *all* ``EsperaQueue`` is used.
    ...
    def __init__(self,
        ...
        self._all_delayq = EsperaQueue(
            burst_limit=all_burst_limit,
            time_limit_ms=all_time_limit_ms,
            exc_route=exc_route,
            autostart=autostart)
        self._group_delayq = EsperaQueue(
    ...
    stop.__doc__ = EsperaQueue.stop.__doc__ or ''  # reuse docsting if any
    ...
    def __call__(self, promise, is_group_msg=False):
        ...
        Args:
            ...
            is_group_msg (:obj:`bool`, optional): Defines whether ``promise`` would be processed in
                group*+*all* ``EsperaQueue``s (if set to ``True``), or only through *all*
                ``EsperaQueue`` (if set to ``False``), resulting in needed delays to avoid
